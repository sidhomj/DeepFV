...
# Test 1: Fit with correct number of kernels
print(f"\n[Test 1] Fitting GMM with {n_mixtures} kernels (correct number)")
print("-" * 60)

fv_dl = FisherVectorDL(n_kernels=n_mixtures, feature_dim=feature_dim)
...
plt.title(f'Learned GMM ({n_mixtures} kernels)')
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.legend()
plt.grid(True, alpha=0.3)
...
# Test 2: BIC-based model selection
print(f"\n[Test 2] BIC-based model selection")
print("-" * 60)

fv_dl_bic = FisherVectorDL(feature_dim=feature_dim)
...
plt.title(f'BIC Selection ({fv_dl_bic.n_kernels} kernels)')
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.legend()
plt.grid(True, alpha=0.3)
